...
models_info = {
    "Adam": {
        ...
        "best_loss_path": os.path.join(BASE_DIR, "results", "weights", "adam", "adam_loss01.npy"),
        "color": "#e74c3c"
    },
    "Momentum": {
        "best_theta": os.path.join(BASE_DIR, "results", "weights", "momentum", "momentum_weights_005.npy"),
        "best_loss_path": os.path.join(BASE_DIR, "results", "weights", "momentum", "momentum_loss_history_005.npy"),
        "color": "#3498db"
    },
    "Nesterov": {
        "best_theta": os.path.join(BASE_DIR, "results", "weights", "Nesterov", "nesterov_weights.npy"),
        "best_loss_path": os.path.join(BASE_DIR, "results", "weights", "Nesterov", "nesterov_loss_05.npy"),
        "color": "#2ecc71"
    },
    "RMSProp": {
        "best_theta": os.path.join(BASE_DIR, "results", "weights", "RMSProp", "rms_weights.npy"),
        "best_loss_path": os.path.join(BASE_DIR, "results", "weights", "RMSProp", "rms_loss_01.npy"),
        "color": "#9b59b6"
    }
}


def main_evaluate_and_plot(X_test_final, X_test_raw, y_test):
    print(" BẮT ĐẦU ĐÁNH GIÁ CÁC THUẬT TOÁN HAND-MADE & SKLEARN...")
    ...
    for name, info in models_info.items():
        try:
            ...
            print(f" Đã tính điểm xong thuật toán: {name}")
        except FileNotFoundError:
            print(f" Không tìm thấy file trọng số tốt nhất của {name}")

    #ĐÁNH GIÁ THEO SKLEARN
    print("  Đang chạy mô hình hồi quy phân vị của Sklearn")
    try:
        # Truyền alpha=0 để tắt chính quy hóa (regularization), đảm bảo công bằng với code tuyến tính của bạn
        # Dùng solver='highs' để thuật toán chạy nhanh và chính xác nhất
        sklearn_model = QuantileRegressor(quantile=0.9, alpha=0.0, solver='highs')
        
        # Sklearn tự động quản lý Bias
        sklearn_model.fit(X_test_raw, y_test)
        y_pred_sk = sklearn_model.predict(X_test_raw)
        
        #chấm điểm dựa trên hàm loss
        ...
        print("  Đã tính điểm xong mô hình thư viện Sklearn.")
...
if __name__ == "__main__":
    ...
    #
    main_evaluate_and_plot(X_test_final, X_test_raw, y_test)